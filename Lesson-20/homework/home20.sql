...
WHERE EXISTS (
    ...
    AND MONTH(sub.SaleDate) = 3 AND YEAR(sub.SaleDate) = 2024
);
...
WHERE Amount < (SELECT MAX(Quantity * Price) FROM #Sales);
...
SELECT * FROM Ancestors ORDER BY ParentId, ChildID;
...
UPDATE #residents
SET fullname = SUBSTRING(address, CHARINDEX('name=', address) + 5, CHARINDEX(' ', address + ' ', CHARINDEX('name=', address) + 5) - CHARINDEX('name=', address) - 5)
...
WITH RoutesCTE AS (
    SELECT CAST(DepartureCity + ' - ' + ArrivalCity AS VARCHAR(MAX)) AS Route, ArrivalCity, Cost
    FROM #Routes
    WHERE DepartureCity = 'Tashkent'
    UNION ALL
    SELECT CAST(r.Route + ' - ' + rt.ArrivalCity AS VARCHAR(MAX)), rt.ArrivalCity, r.Cost + rt.Cost
    FROM RoutesCTE r
    JOIN #Routes rt ON r.ArrivalCity = rt.DepartureCity
)
SELECT Route, Cost FROM RoutesCTE
...
WITH RankCTE AS (
    SELECT ID, Vals,
    COUNT(CASE WHEN Vals = 'Product' THEN 1 END) OVER (ORDER BY ID ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS grp
    FROM #RankingPuzzle
)
SELECT ID, Vals, 
    CASE WHEN Vals <> 'Product' THEN 
        Vals + CAST(ROW_NUMBER() OVER (PARTITION BY grp, Vals ORDER BY ID) AS VARCHAR)
    ELSE Vals END AS RankedValue
FROM RankCTE;
...
WHERE NOT EXISTS (
    SELECT m.MonthNum
    FROM (SELECT DISTINCT SalesMonth AS MonthNum FROM #EmployeeSales) m
    WHERE NOT EXISTS (
        SELECT 1
        FROM #EmployeeSales s
        WHERE s.EmployeeName = e.EmployeeName AND s.SalesMonth = m.MonthNum
    )
);
