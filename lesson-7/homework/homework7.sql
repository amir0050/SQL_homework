...
select category, min(price) as minn, max(price) as maxx from products
group by category

select customerid, count(saleid) as saless
from sales group by customerid



select ProductID, count(saleid) as total, avg(saleid) as avarage from sales
group by ProductID
...
select Departmentname, max(salary) as maxx, min(salary) as minn from Employees
group by Departmentname



select Departmentname, avg(salary) as avarage  from Employees
group by Departmentname
...
select customerid, sum(orderid) from orders
group by customerid 
...
select productid,avg(price) from products group by productid having avg(price)>150

select customerid, sum(saleamount) as total from sales 
group by customerid having sum(saleamount)>1500


select departmentname, avg(salary) as avar, sum(salary) as total from employees
group by departmentname
having avg(salary)>65000


select customerid, sum(case when saleamount>50 then saleamount else 0 end),min(saleamount)from Sales
group by customerid


SELECT strftime('%Y', SaleDate) AS SalesYear,
       strftime('%m', SaleDate) AS SalesMonth,
...
select orderdate, min(quantity),max(quantity) from orders group by orderdate
