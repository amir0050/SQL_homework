...
select*, case when id like '%0%' then '1' else '0' end as flag from SingleOrder
order by id desc

select
isnull(ssn,isnull(passportid,itin)) as non from person

select*from employees

select customerid from orders 
where deliverystate ='ca' and customerid in (select distinct customerid from orders where deliverystate='tx')

select product,unitprice,effectivedate from products where unitprice>100

update employees
alter concat(first_name,' ',last_name) as fullname

SELECT First_Name, Last_Name FROM Employees
WHERE (First_Name + ' ' + Last_Name) LIKE '%a%'
GROUP BY First_Name, Last_Name
HAVING LEN(REPLACE(LOWER(First_Name + ' ' + Last_Name), 'a', '')) = LEN(LOWER(First_Name + ' ' + Last_Name)) - 3;

select count(employee_id),job_id from employees
group by job_id
having percent(first_name where hire_date 
...
WITH RankedExperience AS (
    SELECT
        ...
        JobDescription,
        HireDate,
        ROW_NUMBER() OVER (PARTITION BY JobDescription ORDER BY HireDate ASC) AS RnkAsc,
        ROW_NUMBER() OVER (PARTITION BY JobDescription ORDER BY HireDate DESC) AS RnkDesc
    FROM
        Personal
)
SELECT
    JobDescription,
    MAX(CASE WHEN RnkAsc = 1 THEN SpacemanID END) AS MostExperiencedSpacemanID,
    MAX(CASE WHEN RnkAsc = 1 THEN HireDate END) AS MostExperiencedHireDate,
    MAX(CASE WHEN RnkDesc = 1 THEN SpacemanID END) AS LeastExperiencedSpacemanID,
    MAX(CASE WHEN RnkDesc = 1 THEN HireDate END) AS LeastExperiencedHireDate
FROM
    RankedExperience
WHERE
    RnkAsc = 1 OR RnkDesc = 1
GROUP BY
    JobDescription
ORDER BY
    JobDescription;
...
SELECT
    ID,
    ValueColumn, 
    SUM(ValueColumn) OVER (ORDER BY ID ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS CumulativeSum
FROM
    Students;
...
SELECT
    EquationText,
    CASE
        WHEN CHARINDEX('+', EquationText) > 0 THEN
            CAST(SUBSTRING(EquationText, 1, CHARINDEX('+', EquationText) - 1) AS INT) +
            CAST(SUBSTRING(EquationText, CHARINDEX('+', EquationText) + 1, LEN(EquationText)) AS INT)
        WHEN CHARINDEX('-', EquationText) > 0 THEN
            CAST(SUBSTRING(EquationText, 1, CHARINDEX('-', EquationText) - 1) AS INT) -
            CAST(SUBSTRING(EquationText, CHARINDEX('-', EquationText) + 1, LEN(EquationText)) AS INT)
...
FROM
    Student s1
JOIN
    (SELECT Birthday FROM Student GROUP BY Birthday HAVING COUNT(*) > 1) s2
ON
